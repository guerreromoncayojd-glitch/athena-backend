...
def _get_or_create_equipo(db: Session, team_data: dict, liga: Liga) -> Equipo:
    ...
    if not eq:
        ...
        eq = Equipo(
            ...
            velocidad_juego=round(random.uniform(5.0, 8.0), 1),
            fortaleza_mental=round(random.uniform(5.0, 8.0), 1),
            nivel_presion=round(random.uniform(4.0, 7.5), 1),
            juego_aereo=round(random.uniform(4.0, 7.5), 1),
            juego_bandas=round(random.uniform(4.0, 7.5), 1),
            transiciones_ofensivas=round(random.uniform(4.0, 7.5), 1),
            intensidad=round(random.uniform(4.5, 8.0), 1),
